#!/usr/bin/env python3
"""ROS2 port of right_das_controller_infer.py — same topic mapping as ROS1."""
import sys
...
class GripperDataConverter:
    def __init__(self, node: Node):
        ...
        node.create_subscription(Float32, "/right_gripper/encoder", self.right_gripper_data_callback, QOS_DEFAULT)
        self.right_gripper_feedback_pub = node.create_publisher(
            PoseStamped, "/gripper/right/current_distance", QOS_DEFAULT
        )

        node.create_subscription(
            PoseStamped, "/target_gripper/right_gripper", self.right_cmd_callback, QOS_DEFAULT
        )
        self.right_gripper_cmd_pub = node.create_publisher(
            Float32, "/right_gripper/target_distance", QOS_DEFAULT
        )
    ...
    def publish_all_data(self):
        if self.latest_right_data is not None:
            self.process_gripper_feedback(
                self.latest_right_data, self.right_gripper_feedback_pub, "right"
            )
